...
class OptimizedWaferGeometry:
    ...
    def __init__(
        ...
        if self.effective_radius_mm <= 0:
            raise ValueError(
                f"Effective radius must be positive. Got {self.effective_radius_mm}mm"
            )

        if notch_depth_mm < 0:
            raise ValueError(
                f"Notch depth must be non-negative. Got {notch_depth_mm}mm"
            )

        if notch_depth_mm >= self.wafer_radius_mm:
            raise ValueError(
                f"Notch depth ({notch_depth_mm}mm) cannot exceed wafer radius ({self.wafer_radius_mm}mm)"
            )
    ...
    def _is_rectangle_intersecting_notch(
        ...
        if self.notch_type == NotchType.V_NOTCH_90:
            ...
            return (
                rect_bottom >= notch_y
                and rect_left <= notch_half_width
                and rect_right >= -notch_half_width
                and rect_bottom <= self.wafer_radius_mm
            )
            ...
            return (
                rect_bottom >= notch_y
                and rect_left <= flat_half_width
                and rect_right >= -flat_half_width
                and rect_bottom <= self.wafer_radius_mm
            )

        return False

    def is_rectangle_in_wafer(
        self,
        center_x: float,
        center_y: float,
        width: float,
        height: float,
        method: ValidationMethod = ValidationMethod.CORNER_BASED,
    ) -> Tuple[bool, float]:
        """
        优化版矩形在晶圆内的检查
        """
    ...
    def _validate_corner_based(
        ...
        # 如果最远角都在圆内，则所有角都在圆内
        if (
            far_corner_x * far_corner_x + far_corner_y * far_corner_y
            <= self.effective_radius_mm * self.effective_radius_mm
        ):
            return True
...
class DieCalculator:
    """Alias for OptimizedDieCalculator for backward compatibility."""

    def __init__(self, enable_optimizations: bool = True):
        self._calculator = OptimizedDieCalculator()

    def calculate_dies_per_wafer(self, **kwargs):
        return self._calculator.calculate_dies_per_wafer(**kwargs)
...
class OptimizedDieCalculator:
    ...
    def calculate_dies_per_wafer(
        ...
        self.logger.info(
            f"Calculating DPW (OPTIMIZED): Die {die_size_x_um}×{die_size_y_um}μm, "
        ...
        wafer_geom = OptimizedWaferGeometry(
            wafer_diameter_mm, edge_exclusion_mm, notch_type, notch_depth_mm
        )
        ...
        total_wafer_area_mm2 = (
            math.pi * wafer_geom.effective_radius_mm * wafer_geom.effective_radius_mm
        )
        total_die_area_mm2 = valid_dies * die_size_x_mm * die_size_y_mm
        wafer_utilization = (
            (total_die_area_mm2 / total_wafer_area_mm2) * 100.0
            if total_wafer_area_mm2 > 0
            else 0.0
        )
        ...
        self.logger.info(
            f"Optimized calculation complete: {valid_dies} total dies, "
    ...
    def _calculate_valid_dies_optimized(
        self,
        wafer_geom: OptimizedWaferGeometry,
        pitch_x: float,
        pitch_y: float,
        ...
        if has_notch:
            ...
            for i in range(-max_i, max_i + 1):
                ...
                # 计算对应的有效y范围
                remaining_radius_sq = (
                    effective_radius * effective_radius - center_x * center_x
                )
            ...
            for i in range(max_i + 1):
                ...
                for j in range(max_j_row + 1):
                    ...
                    if is_valid:
                        # 利用对称性计算芯片数量
                        sym_positions = self._get_symmetric_positions(
                            i, j, center_x, center_y, pitch_x, pitch_y
                        )

                        for pos_idx, (sym_i, sym_j, sym_x, sym_y) in enumerate(
                            sym_positions
                        ):
                            distance_from_center = math.sqrt(
